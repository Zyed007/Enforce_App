...
export default class CheckInScreen extends React.Component {
  ...
  errortimer = async () => {
    setTimeout(() => {
      ...
      //     this.setState({ faceReportPopup: true , isLoading : false });
      //     console.log("Display Popup",this.state.faceReportPopup);
          Alert.alert('Request Timeout','You are Exceeded the refresh try Kindly please report the issue',[
          {
            text: 'Report the problem',
            onPress: () => this.ReqsubmitReport(),
            style:'cancel'
          },]);
      }}
    },50000)
  ...
  submitReport = async () => {
    ...
    console.log(params);
    const full_name = `${employeeDetails.full_name} Reported the check-in porblem successfully`;
        ...
        .then((data) => {
          ...
          then(async data => {
            ...
            if (apiResponseData.status == "200") {
              ...
              console.log("Sucess")
               Alert.alert('Face Error Occured','We have failed to recoginize your face and submit the report ',[
                  {
                    text: 'Submit The report',
                    onPress: () => this.facereport(),
                    style:'cancel'
                  },]);
            ...
            const full_name = `${employeeDetails.full_name} Reported the check-in problem successfully`;
            Toast.show(full_name,Toast.LONG);
          }).catch(err => {
            console.log('err--->', err);
            reject(err);
  ...
  verifyFace = async (base64) => {
    ...
    try {
      ...
      const faceResult = faceVerifyResult.FaceMatches;
      console.log("counter face",counter_face)
    if(counter_face<2){
      if (
        faceVerifyResult.statusCode === 400 &&
        !this.state.showAlertIdNoFace
  ...
  addTimesheetCheckIn = async () => {
    ...
    const params = {
      ...
      teamid: teamId,
      check_in: moment(new Date()).utc(true).format("DD/MM/YYYY hh:mm a"),
  ...
  /**
   * Handle Department, designation, outsource, freelance based on index
   * @param {number} index
   */
  ...
  fetchAllProjectByOrgID = async () => {
    ...
    if (apiResponseData) {
      ...
      const projectName = await getProjectData.map((value) => {
        return {
          ...value,
      ...
      const employeeTeam = [...this.state.teamMemberEmpId];
      employeeTeam.push(orgId.id);
      let officeArray = [officeAddress].map(function (item, index) {
        return { label: item.org_name, value: index };
      });
  ...
  renderPlaceTabView() {
    ...
    return (
      <View>
        ...
        />
        {isOffice && !isPlace && officeAddressPlace.length > 0 ? (
          <DropDownPicker
            zIndex={10000}
            items={officeAddressPlace}
            defaultValue={0}
            controller={(instance) => (this.controller = instance)}
            placeholder="Select office"
            searchable={true}
            searchablePlaceholder="Search for office"
            searchablePlaceholderTextColor="gray"
            seachableStyle={{}}
            ...
            onChangeItem={(item, index) =>
              this.onSelectOfficeLocation(item, index)
            }
          />
        ) : (
          <LocationText
