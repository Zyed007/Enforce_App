 ...
 * User navigates to Face registration screen
 * User need to capture 5 photos of the user to get registered
 * After clicking save the images will be stored on the AWS bucket and validate the image.
 * If the user successfully registered, user navigates to home screen.
 * Only one time the user need to register the images
 * API - Patch - BaseUrl.API_BASE_URL + Endpoint.SAVE_FACEID_REGISTRATION is called to store the Face id which we get from the AWS server.
 * 
 */
export default class FaceRegistrationScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      flash: "off",
  ...
  registerFace = async () => {
    ...
    this.userName = employeeDetails.full_name;
    this.setState({ loading: true });
    this.saveFaceReg();
  };
  getImageAndName = async (index) => {
    const { userId, paramsData } = this.state;
    const employeeDetails = await getData(LocalDBItems.employeeDetails);
    ...
    var full_name = employeeDetails.full_name.replace(/[ ]+/g, "");
    ...
    <View style={styles.content}>
      ...
      >
        <Text style={styles.successMessage}>
          Your face recogonize is successfully configured!
        </Text>
      </View>
      <View
        style={{
  ...
  saveFaceIDRegistration = async (faceId) => {
    const employeeDetails = await getData(LocalDBItems.employeeDetails);
    let params = {};
    params = { empid: employeeDetails.id, faceid: faceId };
    ...
    setTimeout(() => {
      ...
      if (apiResponseData) {
        ...
        updatedDetails.is_face_recog = true;
        storeData(LocalDBItems.employeeDetails, updatedDetails);
        if (this.modalizeRef.current) {
          Toast.show("Working with modal");
          this.modalizeRef.current.open();
          this.navigateToDashboard();
        } else {
          Toast.show("Issue with modal");
        }
        // }
      }
    }, 2500);
